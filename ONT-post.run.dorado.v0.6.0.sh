...
GITdir='/data/GIT/';
dorado="$GITdir"/'dorado-0.6.0-linux-x64/bin/dorado';
...
READSin=fastq_pass/"$bc"/;
DEMUXout=temp_demuxed/"$name"."$bc"."$FLOWCELL"/;
READSout="$BCreads"/"$name"."$bc"."$FLOWCELL".fastq.gz;
READSTRIMin="$BCreads"/"$name"."$bc"."$FLOWCELL".fastq.gz;
READSTRIMout="$TRIM"/"$name"."$bc"."$FLOWCELL"."$KITNAME".trimmed.fastq.gz;
...
mkdir -p 01a_bc_reads/"$name"."$bc"."$FLOWCELL"/;

cp fastq_pass/"$bc"/*.fastq.gz 01a_bc_reads/"$name"."$bc"."$FLOWCELL"/;


## demux fastq and trim barcodes and adapters from reads
	"$dorado" demux -v --output-dir "$DEMUXout" --kit-name "$KITNAME" --threads "$NODES" --emit-fastq 01a_bc_reads/"$name"."$bc"."$FLOWCELL"/ ;

## gzip output of dorado
cat "$DEMUXout"/unclassified.fastq "$DEMUXout"/*"$bc".fastq | gzip > "$READSTRIMout";
rm -f "$DEMUXout"/*.fastq;
